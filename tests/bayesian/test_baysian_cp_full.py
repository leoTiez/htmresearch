...
class BayesianCPTest(unittest.TestCase):
    ...
    def test_two_toys_third_sensation_and_same_loc_and_intersections_l2(self):
        # 4 sensations in a sequence fixed
        objects = {
            "simple": [
                # location, feature for CC0
            ...
            "simple2": [
                # location, feature for CC0
                {0: (set([1, 2, 3]), set([1, 14, 15]))},
                {0: (set([16, 17, 18]), set([16, 17, 18]))},
                {0: (set([7, 8, 9]), set([7, 8, 9]))},
                {0: (set([10, 11, 12]), set([10, 20, 21]))},
            ],
        }
        # learn the sensations
            ...
            def test_two_toys_last_sensation_common_l2(self):
                ...
                for o in ["simple", "simple2"]:
                    ...
                    for i in range(len(sensationStepsSingleColumn)):
                        ...
                        print("Sensation %s" % i)
                        print("Overlap %s %s/%s %s" % (
                        "simple", len(overlap), len(simpleObjectRepresentation), overlap))
                        print("Overlap %s %s/%s %s" % (
                            "simple2", len(overlap2), len(simpleObjectRepresentation2), overlap2))
                        print("L2 (object) prediction", objectPrediction)
                        print("L2 activation", active)
    ...
    def test_two_numenta_objects(self):
        ...
        objects = {
            'complex': self.objectMachine.provideObjectsToLearn()[0],
            'complex2': self.objectMachine.provideObjectsToLearn()[1],
        }

        # For manual testing
        # objectsSwitched = {'complex': self.objectMachine.provideObjectsToLearn()[1],
        #                     'complex2': self.objectMachine.provideObjectsToLearn()[0]}
        #
        # objectsSameSensation = {
        #     'complex': [
        #         self.objectMachine.provideObjectsToLearn()[0][0],
        #         self.objectMachine.provideObjectsToLearn()[0][1],
        #         self.objectMachine.provideObjectsToLearn()[0][2],
        #         # self.objectMachine.provideObjectsToLearn()[0][4],
        #     ],
        #     'complex2': [
        #         self.objectMachine.provideObjectsToLearn()[1][0],
        #         self.objectMachine.provideObjectsToLearn()[1][1],
        #         self.objectMachine.provideObjectsToLearn()[1][2],
        #         # self.objectMachine.provideObjectsToLearn()[1][4],
        #     ]
        # }
        ...
        printer = pp.PrettyPrinter(depth=4)

        printer.pprint("Objects")
        printer.pprint(objectsUsed)

        self.exp1.learnObjects(objectsUsed)
        self.exp1.sendReset()
        ...
        print "Object Representations 1 & 2", simpleObjectRepresentation, simpleObjectRepresentation2

        for o in ["complex", "complex2"]:

            sensationStepsSingleColumn = objectsUsed[o]
            objectRepresentation = self.exp1.objectL2Representations[o][0]
